...
namespace WindowsPhoneTestFramework.Server.AndroidDeviceController
{
    public class AdbDeviceControllerConfiguration : ParsedObject
    {
        ...
        {
            get { return Path.Combine(SdkPath, "platform-tools", "adb"); }
        }

        public string EmulatorPath
        {
            get { return Path.Combine(SdkPath, "tools", "emulator"); }
        }

        public string RunningEmulatorAdbName
        {
            get { return "emulator-" + ConsolePort; }
        }

        public string RunningEmulatorWindowName
        {
            get { return string.Format("{0}:{1}", ConsolePort, AvdName); }
        }
    }
}