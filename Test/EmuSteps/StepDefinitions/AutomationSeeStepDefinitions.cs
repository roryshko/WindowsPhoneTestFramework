...
namespace WindowsPhoneTestFramework.Test.EmuSteps.StepDefinitions
{
    ...
    public class AutomationSeeStepDefinitions : AutomationPositionStepDefinitionsBase
    {
        ...
        public void ThenISeeTheNamedFieldWithContent(string namedField, string expectedContents)
        {
            ...
            Assert.IsTrue(result, "Failed to get field contents for '{0}' - looking for '{1}'", namedField,
                          expectedContents);
            Assert.AreEqual(expectedContents, actualContents,
                            "Contents didn't match - field '{0}' - expected '{1}' - actual '{2}'", namedField,
        ...
        {
            string actualValue;
            Assert.IsTrue(Emu.ApplicationAutomationController.WaitForControlOrText(namedField),
                          "Failed waiting for control named {0} to appear", namedField);
        ...
        public void WaitUntilTheTextAppears(string text, string appearance, int maxSeconds)
        {
            ...
            Assert.IsTrue(success, "Did not see the text {0} {1} after waiting {2} seconds", text, appearance,
